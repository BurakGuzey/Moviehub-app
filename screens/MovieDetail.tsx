      ...
      {reviews.length > 0 && (
        <SectionCard title="Reviews">
          <TouchableOpacity onPress={() => setShowReviews(!showReviews)}>
            <Text style={{ color: isDark ? '#ccc' : '#333', marginBottom: 6 }}>
          ...
          {showReviews && reviews.map((review) => (
            <View key={review.id} style={{ marginBottom: 16 }}>
              ...
              <Text numberOfLines={6} style={{ color: isDark ? '#aaa' : '#333', fontSize: 14 }}>
                {review.content}
              </Text>
            </View>
          ))}
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginTop: 12 }}>
            {recommendations.map((rec) => (
              <Pressable key={rec.id} onPress={() => navigation.push('Details', { imdbID: rec.id })} style={{ marginRight: 12 }}>
                <Image
                  source={{ uri: `${TMDB_IMAGE_BASE_URL}${rec.poster_path}` }}
                  style={{ width: 100, height: 150, borderRadius: 8 }}
                />
                <Text numberOfLines={2} style={{ width: 100, marginTop: 6, fontSize: 12, color: isDark ? '#fff' : '#000' }}>
                  {rec.title}
                </Text>
              </Pressable>
            ))}
